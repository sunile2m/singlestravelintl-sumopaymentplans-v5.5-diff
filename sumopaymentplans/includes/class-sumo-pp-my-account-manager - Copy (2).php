...
public static function get_payments_2ton() {
    ...
    try {
        $payments_one = _sumo_pp()->query->get( array(
            'type'       => 'sumo_pp_payments' ,
            //'post_type'       => array('sumo_pp_payments', 'shop_order') ,
            //'posts_per_page' => 1000,
            //'post_status'     => array_keys( 'pending','completed','processing', _sumo_pp_get_payment_statuses() ) ,
        ...
        $allorders = new WP_Query( array (
            ...
            'meta_query'=> array(
                ...
                'value' => get_current_user_id(), // limit to "karma value = 100"
                //'type' => 'numeric'
                )
            ),
        ) ) ;
...
*/
       ...
       $payments = array_splice($payments, 0, 20);


       //$payments = $payments_one;
       array_unique($payments);
       rsort($payments);

/*
echo "<pre>";
print_r($payments_two);
echo "</pre>";
*/
...
*/
        ...
        <?php
        //print_r(array_keys( _sumo_pp_get_payment_statuses() ));
        ?>
        ...
        <table class="shop_table shop_table_responsive my_account_orders <?php echo SUMO_PP_PLUGIN_PREFIX . 'footable' ; ?>" data-filter="#filter" data-page-size="5" data-page-previous-text="prev" data-filter-text-only="true" data-page-next-text="next" style="width:100%">
            ...
            <tbody>
                ...
                foreach( $payments as $payment_id ) :
                    ...
                    //echo "  <p style='margin:0'>payment_id: ".$payment_id."</p>";
                    ...
                    if($o_order_id != $payment_id) {
                        //echo "<b>no, not the same</b>";
                        $notthesame = "true";
                    } else {
                        //echo "<b>yes, the same</b>";
                        $notthesame = "false";
                    }

                    //echo "<h2>o_order_id: ".$o_order_id."</h2>";
                    //echo "<h2>current_o_order_id: ".$current_o_order_id."</h2>";


                    if( ($o_order_id == $current_o_order_id) || $current_o_order_id == "" ) {
                        $hidethis = "hide";
                    } else {
                        $hidethis = "show";
                    }
                    ...
                    $thequery = "SELECT meta_value from wp_i9urzxb0l4_postmeta WHERE meta_key = '_remaining_payable_amount' AND post_id = ".$payment_id."";
                    if ($wpdb->query($thequery) === FALSE) {
                        echo "ERROR!";
                    } else {
                        $thevalue = $wpdb->get_var($thequery);
                        echo "Value: ".$thevalue;
                    }
                    */
                    
                    if($thevalue != "" && $notthesame == "true") {
                        //echo "<h1>hide this</h1>";
                    }
                    ...
                    /*
                    $showit = $wpdb->get_var("SELECT post_type FROM wp_i9urzxb0l4_posts WHERE ID = " .$orderid. " AND post_type = 'shop_order'");

                    $remainingbalance = $wpdb->get_var("SELECT meta_value from wp_i9urzxb0l4_postmeta WHERE meta_key = '_order_total' AND post_id = " . $nextpaymentid);
                    ...
                    return $remainingbalance;
                        ...
                        if a SHOP ORDER 
                        AND
                        HAS a _remaining_payable_amount
                        ...
                        <tr class="<?php echo SUMO_PP_PLUGIN_PREFIX . '-data' ; ?>">
                            ...
                            <td class="<?php echo SUMO_PP_PLUGIN_PREFIX . '-payment-status' ; ?>" data-title="<?php _e( 'Payment Status' , SUMO_PP_PLUGIN_TEXT_DOMAIN ) ; ?>">
                                ...
                                if( $isapayment != "no" && $payment->has_status( 'await_cancl' ) ) {
                                    ...
                                    echo '<mark class="'.$statuslabel.'"/>'.$statuslabel.'</mark>';
                                }
                                ?>
                            </td>
                            <td class="<?php echo SUMO_PP_PLUGIN_PREFIX . '-view-payment' ; ?>">
                                ...
                                <?php
                                if($isapayment == "no") {
                                    echo display_itinerary($o_order_id, $payment_id, 'no');
                                } else {
                                    echo display_itinerary($o_order_id, $payment_id, 'yes');
                                }
                                ?>
                                </div>
